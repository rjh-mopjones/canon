...
3. **canon-core traits** — `Aggregate` (no `handle`/`apply`), `CommandHandler`, `EventHandler`, `Projection`, `CounterfactualReplay`.
3b. **canon-core proc-macros** — all eight macros: `#[aggregate]` → `#[command]` + `#[event]` → `#[event_combiner]` → `#[command_handler]` → `#[event_handler]` → `#[projection]` → `#[projection_handler]`.
...
```
Generates: `impl Aggregate` with version-matched hydration dispatch, `Default`, serde derives, `inventory` registration.
