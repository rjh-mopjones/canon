...
- **Command/event exhaustiveness**: every `#[command(X, version = N)]` must have a `#[command_handler(X, version = N)]`. Every `#[event(X, version = N)]` must have an `#[event_combiner(X, version = N)]`. Missing either is a compile error.
...
- **No casting**: no upcasting or downcasting anywhere. The framework uses version-matched routing — reads `event_version`/`command_version` and dispatches to the handler registered at that version. Each version is processed at the schema it was stored with.
- **`window_ttl` requires `oversight`**: specifying `window_ttl` without an `oversight` method is a compile error. Without `oversight`, the handler always dispatches immediately and a TTL is meaningless.
- **Auto-registration via inventory**: macros emit `inventory`-style static registrations. `ServiceBuilder` discovers all handlers, combiners, and projections automatically — no manual `.register_handler()` calls.
...
pub trait Aggregate: Sized + Send + Sync + 'static {
    ...
    fn hydrate(state: &mut Self::State, events: impl Iterator<Item = EventEnvelope>) -> Result<(), Self::Error> {
        // generated by #[aggregate] macro — reads event_type + event_version,
        // deserializes payload, dispatches to version-matched #[event_combiner] impls
    }
}
...
These are the only macros users interact with. Each is defined in `canon-core`. Users never implement `Aggregate`, `CommandHandler`, `EventHandler`, or `Projection` traits directly — the macros generate all trait impls. All handler and combiner macros emit `inventory`-style static registrations — `ServiceBuilder` discovers them automatically.

### Version-matched routing

There is no upcasting or downcasting anywhere in Canon. The framework reads `event_version` / `command_version` from the stored envelope and dispatches to the handler or combiner registered at that exact version. Each version is processed at the schema it was stored with. This ensures replay (both normal hydration and counterfactual) operates against the same baseline as the original execution.
...
- `impl Aggregate` with hydration dispatch table across all registered `#[event_combiner]` impls for this aggregate (version-matched)
...
### `#[command(AggregateName, version = N, produces = [Events...])]`

Registers a struct as a command belonging to the named aggregate at a specific version. `version` defaults to 1 if omitted. `produces` declares which event types this command can produce — the macro generates a per-command event enum from this list.

Every `#[command(X, version = N)]` must have a corresponding `#[command_handler(X, version = N)]` — missing one is a compile error.
...
// pub enum DepartForStationEvent {
//     ShipDeparted(ShipDeparted),
// }

// Schema evolved in v2 — both versions coexist as different types
#[command(Ship, version = 2, produces = [ShipDeparted, FuelConsumed])]
pub struct DepartForStation {
    pub destination: StationId,
...
### `#[event(AggregateName, version = N)]`

Registers a struct as an event belonging to the named aggregate at a specific version. `version` defaults to 1 if omitted. Both versions can coexist as different types. Every `#[event(X, version = N)]` must have a corresponding `#[event_combiner(X, version = N)]` — missing one is a compile error.
...
// Schema evolved — new field added in v2
...
### `#[event_combiner(AggregateName, version = N)]`

Defines how a specific version of an event folds into aggregate state. One impl block per event type per version. The `combine` method takes `&self` (the event) and `&mut AggregateName` (the state). This is called during hydration — it is synchronous, pure, and has no side effects.
...
### `#[command_handler(AggregateName, version = N)]`

One impl block per command type per version. The `handle` method receives the current aggregate state and the command, and returns a `Result<Vec<CommandEventEnum>>` where `CommandEventEnum` is the per-command enum generated by `#[command]`'s `produces` list. The error type must be declared as an associated item.

During counterfactual replay, the framework reads `command_version` from the stored `CommandEnvelope` and routes to the matching versioned handler.
...
Downstream reactive logic. No aggregate type parameter — event handlers are not tied to a specific aggregate. Optional — not every event needs a handler. The `#[handles]` attribute on the `handle` method declares which event type and version it processes. When a new event version is registered, handlers that don't declare that version receive a compile warning.

Optionally defines `oversight` to control inbox window dispatch readiness. `window_ttl` attribute configures inbox window expiry — but `window_ttl` is only valid alongside an `oversight` method. Specifying `window_ttl` without `oversight` is a compile error.
...
- Every `#[command(X, version = N)]` → must have exactly one `#[command_handler(X, version = N)]` — compile error if missing
- Every `#[event(X, version = N)]` → must have exactly one `#[event_combiner(X, version = N)]` — compile error if missing
- `#[event_handler]` — optional, no exhaustiveness requirement (but compile warning for unhandled new event versions)
- `#[projection_handler]` — optional, no exhaustiveness requirement
- `window_ttl` without `oversight` → compile error
- `#[command_handler]` return type must match the `produces` list from `#[command]` — returning events outside the list is a type error
...
### `ServiceBuilder` auto-registration

All `#[command_handler]`, `#[event_handler]`, `#[event_combiner]`, and `#[projection_handler]` macros emit `inventory`-style static registrations collected at startup. No manual handler registration:
...
Handler registration is automatic via `inventory` — macros emit static registrations, `ServiceBuilder` discovers them at startup.
...
- Events used only to hydrate aggregate state up to the branch point (via version-matched `#[event_combiner]` dispatch)
- Each stored command carries `command_version` — the framework routes it to the `#[command_handler]` registered at that version
...
// Commands — declares which events it can produce
#[command(Ship, version = 1, produces = [ShipDeparted])]
...
// Event combiner — version-matched state folding
#[event_combiner(Ship, version = 1)]
impl ShipDeparted {
    fn combine(&self, state: &mut Ship) {
        state.status = ShipStatus::InFlight;
...
// Command handler — returns the generated per-command event enum
#[command_handler(Ship, version = 1)]
impl DepartForStationHandler {
    type Error = FleetError;
...
// ServiceBuilder auto-discovers all handlers via inventory
ServiceBuilder::new()
    .for_aggregate::<Ship>()
    .build()
```
...
Uses `#[event_handler]` (no aggregate type parameter) with an `oversight` method. `window_ttl` is valid here because `oversight` is present:
