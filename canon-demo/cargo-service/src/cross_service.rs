//! Cross-service event consumer for the cargo service.
//!
...
pub async fn consume_navigation_events(
    ...
    let consumer_id = format!("cargo:cross:{topic}");
    ...
    loop {
        ...
        for record_and_offset in &records {
            ...
            // Derive a deterministic aggregate_id for the manifest from the source
            // event, so replayed events produce the same manifest aggregate.
