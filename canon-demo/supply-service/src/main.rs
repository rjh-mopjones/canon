...
async fn main() -> Result<(), StartupError> {
    ...
    // Per-service schema isolation: supply-service uses canon_supply
    // schema in YugabyteDB and canon_supply keyspace in Cassandra.
    info!("connecting to YugabyteDB at {yugabyte_url} (schema: canon_supply)");
    let yugabyte_pool =
        canon_demo_shared::db::create_service_pool(&yugabyte_url, "canon_supply").await?;
    info!("YugabyteDB connected (schema: canon_supply)");

    info!("connecting to Cassandra at {cassandra_nodes} (keyspace: canon_supply)");
    ...
    info!("Cassandra connected (keyspace: canon_supply)");
    ...
    let outbound_topic = "canon.supply.outbound";

    let es_offset =
        canon_demo_shared::offsets::load_offset(&yugabyte_pool, "supply:es-consumer").await;
    info!(consumer = "supply:es-consumer", offset = ?es_offset, "loaded persisted offset");
    ...
    let es_receiver = canon_demo_shared::offsets::OffsetTrackingReceiver::new(
        es_receiver,
        yugabyte_pool.clone(),
        "supply:es-consumer".to_owned(),
        outbound_topic.to_owned(),
    );

    let proj_offset =
        canon_demo_shared::offsets::load_offset(&yugabyte_pool, "supply:proj-consumer").await;
    info!(consumer = "supply:proj-consumer", offset = ?proj_offset, "loaded persisted offset");
    ...
    let proj_receiver = canon_demo_shared::offsets::OffsetTrackingReceiver::new(
        proj_receiver,
        yugabyte_pool.clone(),
        "supply:proj-consumer".to_owned(),
        outbound_topic.to_owned(),
    );

    let pub_offset =
        canon_demo_shared::offsets::load_offset(&yugabyte_pool, "supply:pub-consumer").await;
    info!(consumer = "supply:pub-consumer", offset = ?pub_offset, "loaded persisted offset");
    ...
    // - Publisher: KafkaPublisher → canon.supply.events
    ...
    let dispatcher_store =
        PgDispatcherStore::new(yugabyte_pool.clone(), event_store.clone(), "Inventory");
    let dispatcher_config = DispatcherConfig {
        batch_size: 100,
        poll_interval_ms: 100,
        aggregate_type_id: std::any::TypeId::of::<Inventory>(),
        max_retries: 3,
    };
    ...
    let cross_service_handle = tokio::spawn(async move {
        info!("cross-service consumer started (canon.station.events)");
