...
An aggregate is the consistency boundary. Version tracking enables optimistic concurrency — the event store rejects writes where the expected version does not match. State is reconstructed by replaying events through `#[event_combiner]` impls. Command handling is owned by `#[command_handler]` impls. Users never implement the `Aggregate` trait directly — the `#[aggregate]` macro generates it.
...
Event schemas evolve over time. `upcast()` is called transparently during hydration to transform raw stored events into the current domain type. The `event_version` field on `EventEnvelope` identifies which schema version a stored event uses.
...
// 4. Define how events fold into state
#[event_combiner(Ship)]
impl ShipRegistered {
...
impl RegisterShipHandler {
    ...
    fn handle(&self, state: &Ship, cmd: RegisterShip) -> Result<Vec<ShipRegistered>, FleetError> {
        Ok(vec![ShipRegistered { name: cmd.name, capacity_kg: cmd.capacity_kg }])
    }
}
...
// 6. Optional — event handlers (no aggregate parameter, reactive downstream logic)
#[event_handler]
...
- `#[event_handler]` and `#[projection_handler]` are optional — no exhaustiveness requirement
...
The `#[event_handler]` macro inspects the `handle` function signature to infer the event type and dispatch strategy, and generates the trait implementation and handler registration boilerplate. The `window_ttl` attribute configures inbox window expiry — e.g. `#[event_handler(window_ttl = "30m")]`.
