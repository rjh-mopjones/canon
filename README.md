...
An aggregate is the consistency boundary. Version tracking enables optimistic concurrency — the event store rejects writes where the expected version does not match. State is reconstructed by replaying events through version-matched `#[event_combiner]` impls. Command handling is owned by `#[command_handler]` impls. Users never implement the `Aggregate` trait directly — the `#[aggregate]` macro generates it.
...
There is no upcasting or downcasting in Canon. Event and command schemas evolve by registering new versions — `#[event(Ship, version = 2)]` and `#[event_combiner(Ship, version = 2)]`. During hydration, the framework reads `event_version` from each stored `EventEnvelope` and dispatches to the combiner registered at that exact version. Each version is processed at the schema it was stored with. This ensures replay always operates against the original baseline.

---

## Macro surface

Canon provides a proc-macro surface that covers the entire domain authoring experience. Users never implement the underlying traits directly — the macros generate all trait impls. Each macro does exactly one job.

### Complete example — Ship aggregate

```rust
// 1. Define the aggregate and its state
...
// 2. Define commands — versioned, with produces list
#[command(Ship, version = 1, produces = [ShipRegistered])]
pub struct RegisterShip { pub name: String, pub capacity_kg: u32 }

#[command(Ship, version = 1, produces = [ShipDeparted])]
pub struct DepartForStation { pub destination: StationId }

// 3. Define events — versioned
#[event(Ship, version = 1)]
pub struct ShipRegistered { pub name: String, pub capacity_kg: u32 }

#[event(Ship, version = 1)]
pub struct ShipDeparted { pub destination: StationId, pub fuel_at_departure: f32 }

// 4. Define how events fold into state — version-matched
...
// 8. ServiceBuilder auto-discovers all handlers via inventory
ServiceBuilder::new()
    .for_aggregate::<Ship>()
    .build()
```
...
pub struct EventEnvelope {
    ...
    pub event_version: u32,       // used for version-matched routing
    pub payload: Bytes,
...
Handler registrations are discovered at startup via `#[event_handler]` macro scanning. The inbox is seeded with the handler manifest so it knows which windows to track.
...
The `#[event_handler]` macro inspects the `handle` function signature to infer the event type and dispatch strategy, and generates the `EventHandler` trait impl and `inventory` registration. The `#[handles]` attribute declares the event type and version. `window_ttl` configures inbox window expiry — but requires an `oversight` method (compile error without one).
...
- Uses events only to hydrate aggregate state up to the branch point (via version-matched `#[event_combiner]` dispatch)
...
3. Apply events via version-matched `#[event_combiner]` dispatch to reach current state.
...
|---|---|
| Aggregate hydration + `#[event_combiner]` | All domain services |
...
| Version-matched routing | Cargo manifest v1 → v2 schema evolution |
