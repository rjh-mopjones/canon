...
Use semantic code-navigation tools first when available: go-to-definition, find-references,
...
- **Never checkout other branches in the main working directory.** Always use git worktrees for branch work, and create them under `~/worktrees/` rather than alongside the main repo. Checking out branches directly causes dist file conflicts, merge conflicts with agent worktrees, and lost work. The main working directory must always stay on `main`.
